...
# Banner (grafika) s barevnými kódy
syringe = f"""
       {yellow}_____{reset}
       {yellow}__H__{reset}
        ["]
...
# Vytiskne banner při spuštění serveru
print(syringe)
...
def execute():
    ...
    # Pokud není příkaz 'data_dump', 'delete' ani 'gedit', spustí standardní příkaz
    try:
...
def data_dump():
    ...
    # Vytvoří tarball (archiv) složky 'data' do souboru 'backup.tar.gz'
    try:
